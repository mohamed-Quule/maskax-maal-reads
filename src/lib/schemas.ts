...
export const bookSchema = (lang: Lang) =>
  z.object({
    ...
    category_id: z.string({ required_error: messages.category(lang) }).uuid(messages.category(lang)),
    cover_url: optionalUrlSchema(lang),
    cover_type: z.enum(["hard", "soft"], { errorMap: () => ({ message: messages.bookType(lang) }) }),
